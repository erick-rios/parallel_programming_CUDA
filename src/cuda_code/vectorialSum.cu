 ...
 * @brief CUDA program to perfom vectorial addition on the GPU.
 * 
 * This program performs vectorial addition on the GPU. The program takes two vectors as input and returns the sum of the two vectors.
 ...
 * @brief Main function.
 * 
 * This function initializes the vectors, allocates memory on the GPU, copies the vectors to the GPU, performs the vectorial addition, copies the result to the CPU, and prints the result.
...
int main(){
    ...
    // Perform the vectorial addition.
    vectorialSum<<<(size + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE>>>(d_vectorA, d_vectorB, d_vectorC, size);
